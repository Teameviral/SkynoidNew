...
if PMPERMIT_PIC is None:
    WARN_PIC = "https://telegra.ph/file/9cf22d3be493d3969e64f.jpg"
else:
...
if lang == "si":
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query.startswith("SkynoidNew"):
            rev_text = query[::-1]
            ...
            result = builder.article(
                title="Stats",
                text=f"**Showing Stats For {DEFAULTUSER}'s SkynoidBot** \nNote --> Only Owner Can Check This \n(C) [Skynoid](https://github.com/TeamEviral/SkynoidNew)",
                buttons=[
                    [custom.Button.inline("Show Stats ?", data="terminator")],
                    [Button.url("Developed By", "https://github.com/TeamEviral")],
                    [Button.url("Support Chat❤️", "t.me/SkynoidHelp")],
                ],
            )
            await event.answer([result])
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        reply_pop_up_alert += "\n\n**(C) SkynoidNew ** ".format(plugin_name)
    ...
    async def rip(event):
        ...
        link_s = yt_dl_data
        if event.query.user_id != bot.uid:
            text = f"Please Get Your Own Skynoid And Don't Waste My Resources"
            await event.answer(text, alert=True)
            return
        is_it = True
        ...
        if event.query.user_id == bot.uid and query.startswith("SkynoidNew"):
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [
                        Button.url(
                            "Repo Here", "https://github.com/TeamEviral/Skynoid"
                        )
                    ],
                    [Button.url("Join Channel ❤️", "t.me/https://t.me/SkynoidUserbot")],
                ],
            )
            await event.answer([result])
        elif event.query.user_id == bot.uid and query.startswith("**Hello"):
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        reply_pop_up_alert = help_string
        reply_pop_up_alert += "\n\n**(C) @SkynoidUserbot** ".format(plugin_name)
        if len(reply_pop_up_alert) >= 4096:
            crackexy = "`Pasting Your Help Menu.`"
            await event.answer(crackexy, cache_time=0, alert=True)
            out_file = reply_pop_up_alert
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy Skynoid To Get Your Own Assistant, Repo Link [Here](https://github.com/TeamEviral/SkynoidNew)",
        )
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy Fire-X To Get Your Own Assistant, Repo Link [Here](https://github.com/TeamEviral/SkynoidNew)",
        )
        await event.answer([resultm])
        return
    testinput = event.pattern_match.group(1)
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="- Not Allowded -",
            text=f"You Can't Use This Bot. \nDeploy Skynoid To Get Your Own Assistant, Repo Link [Here](https://github.com/FireXbot/Fire-X)",
        )
        await event.answer([resultm])
        return
    results = []
